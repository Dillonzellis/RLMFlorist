...
// Update with higher priority to ensure it runs after other field modifications
add_filter('woocommerce_checkout_fields', 'customize_date_of_delivery_field', 99999);
function customize_date_of_delivery_field($fields)
{
  // Check all possible locations for the field
  if (isset($fields['order']['order_date_of_delivery'])) {
    $fields['order']['order_date_of_delivery']['type'] = 'date';
    $fields['order']['order_date_of_delivery']['custom_attributes'] = array(
      'min' => date('Y-m-d'),
      'pattern' => 'd/m/Y', // This hint suggests day/month/year format
      'placeholder' => 'DD/MM/YYYY' // Visual hint for users
    );
  }
  // Check billing section
  else if (isset($fields['billing']['order_date_of_delivery'])) {
    $fields['billing']['order_date_of_delivery']['type'] = 'date';
    $fields['billing']['order_date_of_delivery']['custom_attributes'] = array(
      'min' => date('Y-m-d'),
      'pattern' => 'd/m/Y',
  ...
  else if (isset($fields['shipping']['order_date_of_delivery'])) {
    $fields['shipping']['order_date_of_delivery']['type'] = 'date';
    $fields['shipping']['order_date_of_delivery']['custom_attributes'] = array(
      'min' => date('Y-m-d'),
      'pattern' => 'd/m/Y',
      'placeholder' => 'DD/MM/YYYY'
    );
